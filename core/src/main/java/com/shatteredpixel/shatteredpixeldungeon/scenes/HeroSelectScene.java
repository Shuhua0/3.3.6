...
private class HeroBtn extends StyledButton {
        ...
        HeroBtn ( HeroClass cl ){
            ...
            // 1. 원본 소스에서 48x60 영역을 잘라 독립된 이미지 객체로 생성
            icon( new Image( cl.spritesheet(), 0, 0, 48, 60 ) );
        }

        @Override
        public void draw() {
            if (icon != null) {
                // 2. 버튼 안에서만 0.25배로 줄여서 출력
                // StatusPane에서 1.0f로 복구해주기 때문에 이제 안심하고 줄여도 됩니다.
                icon.scale.set(0.25f);
                icon.origin.set(24, 30);
            }
        ...
        protected void layout() {
            super.layout();
            // 3. 버튼 높이에 맞춰 아이콘의 Y축 정위치 잡기
            if (icon != null && height > 30) {
        ...
        public void update() {
            super.update();
            if (icon != null) {
                // 해금 여부 및 선택 여부에 따른 밝기 조절 (기능 유지)
                icon.brightness(cl == GamesInProgress.selectedClass ? 1f : (cl.isUnlocked() ? 0.6f : 0.1f));
            }
        }
